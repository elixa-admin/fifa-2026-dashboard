...
const FLAG_WIDTHS = [40, 80, 160, 320, 640, 1280];
...
export default function TeamFlag({
  ...
  const px = SIZES[size];
  const flagWidth = FLAG_WIDTHS.find((width) => width >= px * 2) ?? FLAG_WIDTHS[FLAG_WIDTHS.length - 1];
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <div className={`relative p-1.5 ${shellClass}`} style={{ minWidth: px * 1.5 + 12 }}>
        ...
        >
          <Image
            src={`https://flagcdn.com/w${flagWidth}/${team.flagCode}.png`}
            alt={team.name}
            fill
